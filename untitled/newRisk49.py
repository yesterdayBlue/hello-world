...
def openBrowser(username,password):
    browser.find_element_by_xpath("//*[@placeholder='请输入用户名']").send_keys(username)
    browser.find_element_by_xpath("//*[@placeholder='请输入密码']").send_keys(password)
    browser.find_element_by_tag_name('button').click()


def newRisk(filename,path,language):
    browser.implicitly_wait(10)
    # 当控件未加载完成，等待10s
    ...
    # 填写任务名称
    browser.find_element_by_xpath("//input[@value='"+language+"']").click()

    sleep(3)
    browser.find_element_by_class_name('btn-default').click()
    sleep(1)
    os.system('./closeWindow.exe')
    '''k = PyKeyboard()
    # k.tap_key(k.esc_key)
...
def newRisks(rootdir,username,password,language):
    ...
    for i in range(0, len(list)):
        ...
        if os.path.isfile(path):
            ...
            newRisk(os.path.basename(filename),rootdir,language)
    browser.quit()
...
'''
    newRisks(rootdir,username,password,language)
        rootdir: 源码包统一路径 如：E:\\Open source project sample\\finish  注：路径分隔符需要为'\\'
        username：用户名 如：zhaoyiwei
        password：密码 如：ct123!@#
        language: 所发起任务的语言选择 c/c++:1 Object-C:5 C#:2 Java:0 PHP:4 Python:3 Cobol:6
    
'''
newRisks("E:\\Open source project sample\\finish","zhaoyiwei","ct123!@#","0")
